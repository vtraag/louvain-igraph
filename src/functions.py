...
def slices_to_layers(G_coupling,
    ...
    for v_slice in G_coupling.vs:
        ...
        if not vertex_id_attr in H.vertex_attributes():
            raise ValueError(
                "Could not find the vertex attribute {0} to identify nodes in different slices.".format(vertex_id_attr))
        ...
        for u_slice in v_slice.neighbors(mode=_ig.OUT):
            if v_slice.index < u_slice.index or G_coupling.is_directed():
                ...
                if len(set(nodes_v)) != len(nodes_v):
                    err = '\n'.join(
                        ['\t{0} {1} times'.format(item, count) for item, count in list(Counter(nodes_v).items()) if
                         count > 1]
                    )
                ...
                if len(set(nodes_u)) != len(nodes_u):
                    err = '\n'.join(
                        ['\t{0} {1} times'.format(item, count) for item, count in list(Counter(nodes_u).items()) if
                         count > 1]
                    )
                    raise ValueError('No unique IDs for slice {0}, require unique IDs:\n{1}'.format(u_slice.index, err))
                common_nodes = set(nodes_v).intersection(set(nodes_u))
                nodes_v = sorted(
                    [v for v in G.vs if v[slice_attr] == v_slice.index and v[vertex_id_attr] in common_nodes],
                    key=lambda v: v[vertex_id_attr])
                nodes_u = sorted(
                    [v for v in G.vs if v[slice_attr] == u_slice.index and v[vertex_id_attr] in common_nodes],
                    key=lambda v: v[vertex_id_attr])
    ...
    for v_slice in G_coupling.vs:
        H = G.subgraph_edges(G.es.select(_within=[v.index for v in G.vs if v[slice_attr] == v_slice.index]),
