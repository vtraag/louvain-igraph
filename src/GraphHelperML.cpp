...
GraphML::GraphML(igraph_t* graph,
...
GraphML::GraphML(igraph_t* graph, vector<double> const& edge_weights, int correct_self_loops)
{
  this->_graph = graph;
  this->_remove_graph = false;
  this->_correct_self_loops = correct_self_loops;
...
GraphML::GraphML(igraph_t* graph, vector<double> const& edge_weights)
{
  this->_graph = graph;
  this->_remove_graph = false;
  if (edge_weights.size() != this->ecount())
...
GraphML::GraphML(igraph_t* graph, vector<size_t> const& node_sizes, int correct_self_loops)
{
  this->_graph = graph;
  this->_remove_graph = false;
  this->_correct_self_loops = correct_self_loops;
...
GraphML::GraphML(igraph_t* graph, vector<size_t> const& node_sizes)
{
  this->_graph = graph;
  this->_remove_graph = false;
  this->set_defaults();
...
GraphML::GraphML(igraph_t* graph, int correct_self_loops)
{
  this->_graph = graph;
  this->_remove_graph = false;
  this->_correct_self_loops = correct_self_loops;
...
size_t GraphML::possible_edges()
{
  return this->possible_edges(this->vcount());
}

size_t GraphML::possible_edges(size_t n)
{
...
void GraphML::set_defaults()
{
  this->set_default_edge_weight();
  this->set_default_node_size();
}

void GraphML::set_default_edge_weight()
{
  size_t m = this->ecount();
...
void GraphML::set_default_node_size()
{
...
void GraphML::cache_neighbour_edges(size_t v, igraph_neimode_t mode)
{
  #ifdef DEBUG
    cerr << "void GraphML::cache_neighbour_edges(" << v << ", " << mode << ");" << endl;
  #endif
  ...
  #ifdef DEBUG
    cerr << "exit void GraphML::cache_neighbour_edges(" << v << ", " << mode << ");" << endl;
  #endif
}

vector<size_t> const& GraphML::get_neighbour_edges(size_t v, igraph_neimode_t mode)
{
  switch (mode)
  {
    case IGRAPH_IN:
...
pair<size_t, size_t> GraphML::get_endpoints(size_t e)
{
...
void GraphML::cache_neighbours(size_t v, igraph_neimode_t mode)
{
  #ifdef DEBUG
    cerr << "void GraphML::cache_neighbours(" << v << ", " << mode << ");" << endl;
  #endif
  size_t degree = this->degree(v, mode);
  #ifdef DEBUG
    cerr << "Degree: " << degree << endl;
  ...
  #ifdef DEBUG
    cerr << "exit void GraphML::cache_neighbours(" << v << ", " << mode << ");" << endl;
  #endif
}

vector< size_t > const& GraphML::get_neighbours(size_t v, igraph_neimode_t mode)
{
  switch (mode)
  {
    case IGRAPH_IN:
 ...
 ********************************************************************************/
size_t GraphML::get_random_neighbour(size_t v, igraph_neimode_t mode)
{
  size_t node=v;
  size_t rand_neigh = -1;
...
/****************************************************************************
  ...
  graph.
*****************************************************************************/
GraphML* GraphML::collapse_graph(MutableVertexPartition* partition)
{
  #ifdef DEBUG
    cerr << "GraphML* GraphML::collapse_graph(vector<size_t> membership)" << endl;
  #endif
  size_t m = this->ecount();
  ...
  GraphML* G = new GraphML(graph, collapsed_weights, csizes, this->_correct_self_loops);
  G->_remove_graph = true;
  #ifdef DEBUG
    cerr << "exit GraphML::collapse_graph(vector<size_t> membership)" << endl << endl;
  #endif
  return G;
}
